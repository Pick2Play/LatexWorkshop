...
In dem ersten Abschnitt der Arbeit geht es um die Problemstellung, die Motivation, die Zielsetzung, die Abgrenzung und die Vorgehensweise. Das zu lösende Problem ist, dass die veraltete Access-Anwendung zu einem erhöhten Arbeitsaufwand führt. 
...
Ziel des Prototyps ist, eine Weboberfläche zu entwickeln die effizienter und benutzerfreundlicher, als die Access Anwendung, für das Unternehmen ist. 
...
Änderungen werden durch LINQ-Abfragen \citeay{BeginningEFCore} durchgeführt, die eine einfache und lesbare Möglichkeit bieten, Datenbankoperationen durchzuführen. Es gibt zwei arten von Syntax, mit der LINQ Queries geschrieben werden können. Zum einen können LINQ Queries in der Methodensyntax \ref{lst:Methodensyntax} geschrieben werden, zum anderen in der Abfragesyntax.

% Stimmt das mit Fluent API so ?
...
\begin{minted}{csharp}
    var persons = from p in Persons
        ...
        select p;
\end{minted}
\caption{Beispiel für eine LINQ Query in Abfragesyntax \citeay{BeginningEFCore}}
\label{lst:Abfragesyntax}
\end{listing}

Die Syntax in Listing \ref{lst:Abfragesyntax} zeigt eine LINQ Query in der Abfragesyntax, die alle Personen mit dem Vornamen „Susan" aus der Personen Tabelle abruft. Die Syntax gleicht einer SQL-Abfrage und ist für Entwickler, die mit SQL vertraut sind, leicht verständlich. Die Methodensyntax verwendet Fluentapi \citeay{BeginningEFCore}, um die Abfrage aneinander zu ketten.
...
\end{minted}
\caption{Beispiel für eine LINQ Query in Methodensyntax \citeay{BeginningEFCore}}
\label{lst:Methodensyntax}
% Springt immer zu einer Quelle wenn ich den Verweise referenziere

\end{listing}

Die Syntax in Listing \ref{lst:Methodensyntax} zeigt eine LINQ Query in der Methodensyntax, die alle Personen mit dem Nachnamen „Smith" aus der Personen Tabelle abruft. In der Abfrage wird eine Arrowfunktion verwendet, um die Query Bedingung zu definieren.

Um die Usability des Prototyps zu verbessern, werden UI- und UX-Designprinzipien angewendet. Dazu gehört die Verwendung von klaren und konsistenten Layouts, intuitiven Navigationselementen und ansprechenden visuellen Designs. Die Entwicklung orientiert sich am Human Centered Design Ansatz \citeay{HCDBook}, um sicherzustellen, dass die Bedürfnisse und Erwartungen der Benutzer berücksichtigt werden.
Damit die UI- und UX-Designentscheidungen fundiert sind, wird auf etablierte Designprinzipien und Heuristiken zurückgegriffen \citeay{UsibilityHeuristics}.



% Entwicklungsentscheidung für Methodensyntax erläutern -> Warum benutze ich diese Syntax ?
% Einmal ToList und Arrowfunktion erklären -> Ich brauche vielleicht eine Separate Quelle für Arrowfunktions !
...
Zum Beispiel könnte die Entscheidung Methodensyntax für LINQ Queries zu verwenden reflektiert werden. Als Alternative hätte der Autor die Abfragesyntax verwenden können. Hier können die Vor- und Nachteile der beiden Syntaxen diskutiert werden. 
Weiterhin können die getroffenen Designentscheidungen reflektiert werden. Wurden die Designprinzipien eingehalten? Wären andere Entscheidungen angemessener gewesene? 
Konnte der Zeitplan eingehalten werden? Gab es unvorhergesehene Herausforderungen? Auch die tatsächliche Zeiteinteilung kann hier im Bezug zum Zeitplan aufgeführt werden.
Außerdem werden Empfehlungen abgeleitet, die bei Projekten in der Zukunft berücksichtigt werden sollten.

% Waren die Designentscheidungen gut? War der Zeitplan realistisch ? 
...
% Ist die Usability durch die Designentscheidung verbessert worden ? -> Gibt es einen anstieg der produktivität der Nutzer ? -> Könnte das auch an der Vertrautheit mit dem alten System liegen ? etc.
